...
def mixpoisson_sample(
    size=1, mus=np.ones(2), weights=np.ones(2) * 0.5, random_state=None
):
    """
    ...
    assert len(mus) == len(weights), (
        "Number of components in mus and weights must be the same"
    )

    # Sample component assignments
    component_samples = random_state.choice(n_components, size=size, p=weights)
...
class PoissonMixture(BaseMixture):
    ...
    def _initialize(self, X, resp, xp=None):
        ...
        self.weights_, self.probs_ = _estimate_poisson_parameters(
    ...
    def _initialize_parameters(self, X, random_state, xp=None):
        ...
        # the initialization.
        compute_resp = self.weights_init is None or self.mus_init is None
        if compute_resp:
            self.weights_, self.mus_ = _estimate_mixpoisson_parameters(
                n_components=self.n_components,
                X=X,
                resp=None,
                random_state=random_state,
    ...
    def _m_step(self, X, log_resp):
        ...
        for k in range(self.n_components):
            ...
            self.mus_[k] = np.clip(
                self.mus_[k], np.finfo(float).tiny, np.finfo(np.float32).max
            )
    ...
    def point_estimate(self):
        ...
        for i in range(self.n_components):
            params_detail[f"m{i}"] = {"pindex": i, "val": self.mus_[i]}
    ...
    def _fisher_tensor(self, i, j):
        ...
        def __partial_logp_partial_weight(x):
            ...
            result = prob_diff / binommix_logpmf(
                x=x,
                n_trials=n_trials,
                params_weights=self.weights_,
                params_probs=self.probs_,
