...
def visualize_joint_confidence_regions(mle_samples, true_params=None, 
    ...
    for i in range(n_params):
        for j in range(i + 1, n_params):
            ...
            for method_idx, method in enumerate(methods):
                ...
                if method == 'scatter':
                    plot_scatter_method(ax, x_data, y_data, param_x, param_y, 
                                      true_params, confidence_levels)
                elif method == 'kde':
                    plot_kde_method(ax, x_data, y_data, param_x, param_y, 
                                  true_params, confidence_levels)
                elif method == 'ellipse':
                    plot_ellipse_method(ax, x_data, y_data, param_x, param_y, 
                                      true_params, confidence_levels)
                elif method == 'hull':
                    plot_hull_method(ax, x_data, y_data, param_x, param_y, 
                                   true_params, confidence_levels)
                ...
                if method_idx == 0:  # Add parameter pair label to leftmost column
                    ax.set_ylabel(f'{param_y} vs {param_x}', fontsize=10, rotation=90, 
                                labelpad=20)
...
def plot_scatter_method(ax, x_data, y_data, param_x, param_y, true_params, confidence_levels):
    """Scatter plot with truth point."""
    ax.scatter(x_data, y_data, alpha=0.6, s=15, color='steelblue', 
               label=f'MLE samples (n={len(x_data)})')
    
    if true_params:
        ax.scatter(true_params[param_x], true_params[param_y], 
...
def plot_kde_method(ax, x_data, y_data, param_x, param_y, true_params, confidence_levels):
    """KDE contour plot."""
    ...
    try:
        ...
        xx, yy = np.mgrid[x_min-x_pad:x_max+x_pad:50j, 
                         y_min-y_pad:y_max+y_pad:50j]
        ...
        for i, conf_level in enumerate(confidence_levels):
            ...
            ax.contour(xx, yy, density, levels=[threshold], 
...
def plot_ellipse_method(ax, x_data, y_data, param_x, param_y, true_params, confidence_levels):
    ...
    try:
        ...
        for i, conf_level in enumerate(confidence_levels):
            ...
            ellipse = Ellipse((mean_x, mean_y), width, height, angle=angle,
                             ...
                             linewidth=2, label=f'{int(conf_level*100)}% CI')
            ax.add_patch(ellipse)
        
        # Mark sample mean
        ax.scatter(mean_x, mean_y, color='blue', s=50, marker='x', 
...
def plot_hull_method(ax, x_data, y_data, param_x, param_y, true_params, confidence_levels):
        ...
        for i, conf_level in enumerate(confidence_levels):
            ...
            if len(inner_points) >= 3:  # Need at least 3 points for hull
                ...
                for simplex in hull.simplices:
                    ax.plot(inner_points[simplex, 0], inner_points[simplex, 1], 
...
def example_monte_carlo_confidence_regions():
    ...
    plt.suptitle('Joint Confidence Regions: Monte Carlo MLE Distribution', 
                 fontsize=16, y=0.98)
    plt.show()
