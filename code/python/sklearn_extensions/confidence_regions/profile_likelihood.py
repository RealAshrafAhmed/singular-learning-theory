    ...
    def intervals(self, params_index=None, confidence_level=0.95):
        ...
        for pindex in range(len(sigmas)):
            ...
            for pvalue in param_linespace:
                profile_ll_values = self.profile_ll(params_index=[pindex], params_value=[self.mle[pindex]])
                threshold = np.max(profile_ll) - chi2.ppf(confidence_level, 1) / 2
                pintervals = profile_ci_regions(grid=param_linespace,
                                                profile_ll=profile_ll_values,
                                                threshold=threshold)

            if len(pintervals) > 1:
                raise Exception(f"Profile likelihood intervals are {len(pintervals)} not 1")
