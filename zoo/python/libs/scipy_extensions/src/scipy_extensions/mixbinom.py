...
def logpmf(weights, probs, n, x):
    ...
    x = np.atleast_1d(x)
    result = binom.logpmf(k=x[:, np.newaxis], n=n, p=probs)
    return logsumexp(result + np.log(weights), axis=1)


def log_likelihood(weights, probs, n, x):
    """
    ...
    result_per_component = binom.logpmf(
        k=x[:, np.newaxis, np.newaxis],  # Shape (N_obs, 1, 1)
        n=n,
        p=probs[np.newaxis, ...]   # Shape (1, N_draws, 2)
    )
