...
class ConfidenceInterval:
    ...
    def contains(self, values):
        ...
        within_intervals = np.all(
            (values >= self.lower_bounds) & (values <= self.upper_bounds)
        )
...
def confidence_estimates(
    truth, data, mle, cov_provider, cr_providers, confidence_levels=[0.90, 0.95, 0.99]
):
    print(f"truth={truth}")
    ...
    for confidence_level in confidence_levels:
        for cr_provider in cr_providers:
            ...
            for k, v in mle.items():
                ...
                pval = v["val"]
                if (
                    pindex < fisher_mat.shape[0]
                ):  # only include the unconstrained parameters
                    cr_estimate.append(
                        {
                            "pname": k,
                            "index": pindex,
