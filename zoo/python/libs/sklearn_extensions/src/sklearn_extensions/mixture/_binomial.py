...
def mixbinom_kl_divergence(
    n_trials: int,
) -> Callable[[List[NDArray[np.float64]], NDArray[np.float64]], NDArray[np.float64]]:
    ...
    def fun(
        references: List[NDArray[np.float64]], queries: NDArray[np.float64]
    ) -> NDArray[np.float64]:
        """
        ...
        log_q = _compute_log_pmf(
            query_probs, query_weights
        )  # (n_queries, n_trials + 1)
...
def mixbinom_divergence(n_trials: int, kind: Divergence = None, allow_marginals=False):
    match kind:
        case Divergence.KL:
            if allow_marginals:
        ...
        case _:  # default to euclidean distance
            ...
            def euclidean_div(
                references: List[np.float64], queries: NDArray[np.float64]
            ) -> NDArray[np.float64]:
...
def mixbinom_logpmf(n_trials, weights, probs, flat=True):
    ...
    assert np.all(negative_weights), (
        f"Weights {weights} must be between 0 and 1 inclusive"
    )
    n_trials = n_trials
...
class BinomialMixture(BaseMixture):
    ...
    def __init__(
        ...
        *,
        ...
        if weights_init is not None and probs_init is not None:
    ...
    def _initialize(self, X, resp, xp=None):
        ...
        self.weights_, self.probs_ = _estimate_binomial_parameters(
    ...
    def _initialize_parameters(self, X, random_state, xp=None):
        ...
        # the initialization.
        compute_resp = self.weights_init is None or self.probs_init is None
        if compute_resp:
            self.weights_, self.probs_ = _estimate_binomial_parameters(
                n_components=self.n_components,
                X=X,
    ...
    def _m_step(self, X, log_resp):
        ...
        resp_sum = (
            np.sum(resp, axis=0) + 10 * np.finfo(resp.dtype).eps
        )  # ensure some numerical stability
        self.weights_ = resp_sum / n_samples

        # adjust the weights percision
        self.weights_ = [
            np.clip(w, self.reg_covar, 1 - self.reg_covar) for w in self.weights_
        ]
    ...
    def sample(self, n_samples=1, random_state=None):
        ...
        component_samples = random_state.choice(
        ...
        for k in range(self.n_components):
            ...
            if n_component_samples > 0:
                ...
                samples[mask] = random_state.binomial(
                    n_trials, self.probs_[k], size=n_component_samples
                )
    ...
    def point_estimate(self):
        ...
        for i in range(self.n_components):
            params_detail[f"p{i}"] = {"pindex": i, "val": self.probs_[i]}
    ...
    def fisher_matrix(self, X):
        d = (
            self.n_components * 2 - 1
        )  # the size of the parameter space. -1 since one mixing component is a function of the others
        m = np.zeros((d, d))
        for i in range(d):
            for j in range(d):
    ...
    def _fisher_tensor(self, i, j):
        ...
        def __partial_logp_partial_prob(x, prob_index):
            ...
            binomial_term = weights[prob_index] * binom.pmf(
                k=x, n=n_trials, p=probs[prob_index]
            )
            score_factor = x / probs[prob_index] - (n_trials - x) / (
                1 - probs[prob_index]
            )
            result = (
                (
        ...
        def __partial_logp_partial_weight(x):
            ...
            prob_diff = binom.pmf(k=x, n=n_trials, p=probs[0]) - binom.pmf(
                k=x, n=n_trials, p=probs[1]
            )
            result = prob_diff / mixbinom_logpmf(
                n_trials=n_trials, weights=self.weights_, probs=self.probs_, flat=True
            )(x)
            return result
        ...
        def __tensor(x):
            ...
            if is_i_prob and is_j_prob:
                ...
                if i == j:
                    ...
                    return __partial_logp_partial_prob(
                        x, i
                ...
                return __partial_logp_partial_weight(x) * __partial_logp_partial_prob(
                    x, j
                )
